...
struct IPCProtocolTests {
    ...
    // MARK: - Event system & script widget commands

    @Test
    func `subscribe command round-trips through Codable`() throws {
        let cmd = IPCCommand.subscribe(events: ["my_event", "other_event"])
        let request = IPCRequest(command: cmd)
        let data = try JSONEncoder().encode(request)
        let decoded = try JSONDecoder().decode(IPCRequest.self, from: data)
        #expect(decoded.command == cmd)
    ...
    func `trigger command round-trips through Codable`() throws {
        let cmd = IPCCommand.trigger(event: "custom_event", env: ["KEY": "value", "FOO": "bar"])
        let request = IPCRequest(command: cmd)
        let data = try JSONEncoder().encode(request)
        let decoded = try JSONDecoder().decode(IPCRequest.self, from: data)
        #expect(decoded.command == cmd)
    ...
    @Test
    func `addItem command round-trips through Codable`() throws {
        let cmd = IPCCommand.addItem(id: "vpn.status", position: .right)
        let request = IPCRequest(command: cmd)
        let data = try JSONEncoder().encode(request)
        let decoded = try JSONDecoder().decode(IPCRequest.self, from: data)
        #expect(decoded.command == cmd)
    ...
    @Test
    func `removeItem command round-trips through Codable`() throws {
        let cmd = IPCCommand.removeItem(id: "vpn.status")
        let request = IPCRequest(command: cmd)
        let data = try JSONEncoder().encode(request)
        let decoded = try JSONDecoder().decode(IPCRequest.self, from: data)
        #expect(decoded.command == cmd)
    ...
    @Test
    func `IPCEvent round-trips through Codable`() throws {
        let event = IPCEvent(event: "test_event", env: ["key": "value"])
        let data = try JSONEncoder().encode(event)
        let decoded = try JSONDecoder().decode(IPCEvent.self, from: data)
        #expect(decoded.event == "test_event")
        #expect(decoded.env == ["key": "value"])
    }

    @Test
    func `event payload round-trips through Codable`() throws {
        let event = IPCEvent(event: "my_event", env: [:])
        let response = IPCResponse(
            requestID: "sub-1",
        ...
        let data = try JSONEncoder().encode(response)
        let decoded = try JSONDecoder().decode(IPCResponse.self, from: data)
        if case let .success(.event(decodedEvent)) = decoded.result {
            #expect(decodedEvent.event == "my_event")
        } else {
            Issue.record("Expected success with event payload")
        }
    }

    @Test
    func `itemAlreadyExists error round-trips`() throws {
        let response = IPCResponse(
            requestID: "r1",
            result: .failure(.itemAlreadyExists(id: "vpn")),
        )
        let data = try JSONEncoder().encode(response)
        let decoded = try JSONDecoder().decode(IPCResponse.self, from: data)
        #expect(decoded.result == .failure(.itemAlreadyExists(id: "vpn")))
    }

    @Test
    func `invalidProperty error round-trips`() throws {
        let response = IPCResponse(
            requestID: "r1",
            result: .failure(.invalidProperty("unknown_prop")),
        )
        let data = try JSONEncoder().encode(response)
        let decoded = try JSONDecoder().decode(IPCResponse.self, from: data)
        #expect(decoded.result == .failure(.invalidProperty("unknown_prop")))
    }
